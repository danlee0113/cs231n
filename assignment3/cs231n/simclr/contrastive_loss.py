...
def sim(z_i, z_j):
    ...
    norm=torch.linalg.norm(z_i)*torch.linalg.norm(z_j)
    norm_dot_product=dot_product/norm
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l1=-torch.log(torch.exp(sim(z_k,z_k_N)/tau)/d1)
        l2=-torch.log(torch.exp(sim(z_k_N,z_k)/tau)/d2)
        total_loss+=l1+l2
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N=out_left.shape[0]
    pos_pairs=torch.sum(out_left*out_right,dim=1).reshape(N,-1)
    pos_pairs/=torch.linalg.norm(out_left,dim=1).reshape(N,-1)
    pos_pairs/=torch.linalg.norm(out_right,dim=1).reshape(N,-1)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def compute_sim_matrix(out):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    norm=torch.linalg.norm(out,dim=1).reshape(out.shape[0],-1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom=exponential.sum(dim=1) #exponential is the index extracted version of sim_matrix.
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    n1=sim_matrix[torch.arange(N),torch.arange(N)+N]
    n2=sim_matrix[torch.arange(N)+N,torch.arange(N)]
    numerator=torch.cat([n1,n2],dim=0)
