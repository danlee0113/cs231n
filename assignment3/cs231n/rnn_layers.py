...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    add_value=prev_h.dot(Wh)+x.dot(Wx)+b
    next_h=np.tanh(add_value)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache=[]
    for t in range(T):
      next_h,cache_tmp=rnn_step_forward(x[:,t,:],h0,Wx,Wh,b)
      h0=next_h
      h[:,t,:]=h0
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    H=prev_h.shape[1]
    a=x.dot(Wx)+prev_h.dot(Wh)+b #doesn't have to be a vector as it says in the assignment notebook.
    
    i=sigmoid(a[:,:H]) #split a into 4 parts.
    f=sigmoid(a[:,H:2*H])
    o=sigmoid(a[:,2*H:3*H])
    g=np.tanh(a[:,3*H:])
    next_c=f*prev_c+i*g
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      h0,c0,cache_tmp=lstm_step_forward(x[:,t,:], h0, c0, Wx, Wh, b)
      h[:,t,:]=h0
      cache.append(cache_tmp)
...
def temporal_affine_forward(x, w, b):
    ...
    N, T, D = x.shape
    M = b.shape[0]
    out = x.reshape(N * T, D).dot(w).reshape(N, T, M) + b #M reperesents the dimension of each time step.
    cache = x, w, b, out
    return out, cache
